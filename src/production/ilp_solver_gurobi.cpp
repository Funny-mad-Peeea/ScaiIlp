...
namespace ilp_solver
{
    ...
    void ILPSolverGurobi::add_constraint_sense(GRBLinExpr&& row, double p_bound, const std::string& p_name, char sense)
    {
        d_model.addConstr(row, sense, p_bound,
    ...
    void ILPSolverGurobi::add_constraint_range(GRBLinExpr&& row, double p_lower_bound, double p_upper_bound, const std::string& p_name)
    {
        ...
        else
        {
            ...
            d_model.addRange(std::move(row), p_lower_bound, p_upper_bound,
#ifdef DO_FORWARD_NAME
            p_name);
#else
            std::string()); p_name;
    ...
    {
        auto new_var = d_model.addVar(p_lower_bound, p_upper_bound, p_objective, p_type,
#ifdef DO_FORWARD_NAME
        p_name);
#else
        std::string()); p_name;
    ...
    void ILPSolverGurobi::add_variable(const std::vector<double>& p_row_values, double p_lower_bound, double p_upper_bound, double p_objective, const std::string& p_name, char p_type)
    {
        ...
        auto new_var = d_model.addVar(p_lower_bound, p_upper_bound, p_objective, p_type, static_cast<int>(d_rows.size()), d_rows.data(), p_row_values.data(),
#ifdef DO_FORWARD_NAME
        p_name);
#else
        std::string()); p_name;
    ...
    void ILPSolverGurobi::add_variable(const std::vector<int>& p_row_indices, const std::vector<double>& p_row_values, double p_lower_bound, double p_upper_bound, double p_objective, const std::string& p_name, char p_type)
    {
        ...
        auto new_var = d_model.addVar(p_lower_bound, p_upper_bound, p_objective, p_type, static_cast<int>(tmp_rows.size()), tmp_rows.data(), p_row_values.data(),
#ifdef DO_FORWARD_NAME
        p_name);
#else
        std::string()); p_name;
